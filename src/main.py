...
def keyboard():
    return ReplyKeyboardMarkup(row_width=3).add(*[KeyboardButton(x) for x in ("-1", "clear", "+1")])


def main_keyboard():
    return ReplyKeyboardMarkup(row_width=1).add(*[KeyboardButton(x) for x in ("/open_class", "/new_class", "/remove_class")])


@bot.message_handler(commands=["help", "start"])
def cmd_main(msg):
    bot.reply_to(msg, """hi there! this is a main menu!""", reply_markup=main_keyboard())


@bot.message_handler(commands=["open_class"])
def cmd_open_class(msg):
    bot.send_message(msg.chat.id, f"Available classes:\n" + "\n".join(map(lambda x: x[1], points.get_classes(msg.chat.id))))
    bot.register_next_step_handler(msg, select_class)


def select_class(msg):
    try:
        selected = next((x[0] for x in points.get_classes(msg.chat.id) if x[1] == msg.text))
        bot.send_message(msg.chat.id, str(points.get_students(selected)))
    except:
...
def cmd_new_class(msg):
    bot.send_message(msg.chat.id, f"Enter the new class name. Or enter \"cancel\" (case-insensitive) to cancel the operation")
